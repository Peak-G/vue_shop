  ...
  methods: {
    ...
    async getParamsDate() {
      ...
      if (this.selectedCateKeys.length !== 3) {
        (this.selectedCateKeys = []), (this.manyTableData = []);
        this.onlyTableData = [];
        return;
      }
      // 证明选中的是三级分类
    ...
    handleInputConfirm(scope) {
      if (scope.inputValue.trim().length == "") {
        scope.inputValue = "";
      ...
      scope.inputVisible = false;
      // 需要发起请求保存这次操作
      this.saveAttrValue(scope);
    },
    // 点击按钮显示输入文本框
    showInput(scope) {
      scope.inputVisible = true;
      // 文本框自动获得焦点
      // $nextTick 方法的作用，就是页面上元素被重新渲染之后才会执行回调函数中的代码
      this.$nextTick((_) => {
        this.$refs.saveTagInput.$refs.input.focus();
      });
    },
    // 将对attr_vals的操作保存到数据库中
    async saveAttrValue(scope) {
      const { data: res } = await this.$http.put(
